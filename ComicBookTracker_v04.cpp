...
bool compareAlphabetical(ComicBookRunInfo a, ComicBookRunInfo b)
{
	return a.getComicTitle() < b.getComicTitle();
}

bool compareCurrentlyOngoing(ComicBookRunInfo a, ComicBookRunInfo b)
{
	return a.getIsOngoing() < b.getIsOngoing();
}

void organize(int userOption, std::array<ComicBookRunInfo, 44>& comicBookRun)
{
	if (userOption == 1)
	{
		std::sort(comicBookRun.begin(), comicBookRun.end(), compareAlphabetical);
	}
	else if (userOption == 2)
	{
		std::sort(comicBookRun.begin(), comicBookRun.end(), compareCurrentlyOngoing);
	}
}

void setData(int& userOption)
{
	std::cout << '\n' << "** Carlos Personal Comic Book Collection **" << '\n' << '\n';

	std::cout << "How would you like your comic data displayed?" << '\n';
	std::cout << "(1)Alphabetical order	(2)Currently ongoing" << '\n';
...
int main()
{
	...
	int userOption{ 0 };							//Where we hold and use "User sorting option"
	...
	organize(userOption, comicBookRun);
